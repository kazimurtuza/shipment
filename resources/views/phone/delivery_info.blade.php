...
@section('content')
    <h1 class="headtx">Scan Delivery  Documents</h1>
    <?php $shipmentdata=\App\Models\shipment::find($shipment_id);
...
;"
    ...
    <form action="{{route('updatedelivery_from_phone')}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row pd" id="phototake">
            ...
            <input type="hidden" name="inv_id"value="{{$shipmentdata->invoice_no}}">
            <input type="hidden" name="shipment_id"value="{{$shipment_id}}">
            <div class="col-12"> <h4 style="font-weight: 700;font-size: 11.5px;line-height: 19px;color: rgba(0, 0, 0, 0.67);">
                    <strong>Pickup address &nbsp;:</strong>&nbsp; {{$shipmentdata->form}}</h4></div>
  ...
  font-weight: 500;">+</span></h5>
            ...
            <h1 style="margin-top:10px;font-weight: 700;font-size: 11px;">Please enter the pickup time  </h1>
            ...
            <div class="col-6" style="position: relative ">
                <label class="labeltx" for="">Delivery date</label>
                <input required type="date" style="width: 140px;" name="delivery_date" value="{{$shipmentdata->delivery_date}}" class="form-control bdradi">
            </div>
            <div class="col-6" style="position: relative">
                <label class="labeltx" for="">Delivery time</label>
                <input required type="time" name="delivery_time" value="{{$shipmentdata->delivery_time}}" class="form-control bdradi">
            </div>

            <div class="col-12 text-center mt-5">
                <button class="btn"  style="background: #2B4BF2;  width: 53%; color:white">Submit</button>
            </div>
...
<td style="position: relative">
    ...
    <input type="hidden" id="bill${randomid}" name="proof_delivery[]">
    <img id="${randomid}" show-data="${randomid}" onclick="viewimg(this)" width="180" height="101.25" src="" alt="Red dot" />
